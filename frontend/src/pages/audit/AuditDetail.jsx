...
import {
  ...
  ChevronDown, ChevronUp, MessageSquare, Eye,
} from 'lucide-react';
...
export default function SupervisionDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await processAPI.listStaff();
        if (alive) setStaffList(s);
      } catch (e) { /* ignore */ }
      try {
        const d = await auditAPI.get(id);
        if (!alive) return;
  ...
  const readOnly = aud?.estado === 'completada';
  ...
  const saveItem = async (itemId, patch) => {
    setSaving(true);
    try {
      await auditAPI.updateItem(id, itemId, patch);
      const d = await auditAPI.get(id);
      setAud(d);
  ...
  const markAllYes = async () => {
    ...
    try {
      ...
      // Backend en paralelo
      await Promise.all(pending.map(it => auditAPI.updateItem(id, it.id, { cumplido: true })));
      // Refrescar header (puntaje/aprobada)
      const d = await auditAPI.get(id);
      setAud(d); setItems(d.items || []);
    } catch (e) {
      alert(e.message);
      reload();
    }
  ...
  const finalize = async () => {
    ...
    } else if (!window.confirm('¿Marcar esta auditoría como completada?')) return;
    setCompleting(true);
    try {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`¿Eliminar ${aud.codigo}?`)) return;
    try {
  ...
  if (loading) return <div className="flex items-center gap-2 text-slate-500"><Loader2 className="w-4 h-4 animate-spin"/>Cargando auditoría...</div>;
  if (!aud) return <div className="text-slate-500">Auditoría no encontrada</div>;
  ...
  return (
    <div className="animate-fade-in max-w-3xl mx-auto" data-testid="audit-detail-page">
      <button onClick={() => navigate('/audit')} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 mb-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs text-slate-400 font-mono mb-1 flex-wrap">
              <span>{aud.codigo}</span>
              <span className="text-slate-300">·</span>
              <span>{aud.proceso_codigo}</span>
              {aud.area_nombre && <span className="text-slate-500">· {aud.area_nombre}</span>}
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${aud.estado === 'completada' ? 'bg-slate-900 text-white border-slate-900' : 'bg-amber-100 text-amber-800 border-amber-200'}`}>
                {aud.estado === 'completada' ? 'Completada' : 'En curso'}
              </span>
            </div>
            <h1 className="text-2xl font-semibold text-slate-900" style={{ fontFamily: 'Outfit' }}>{aud.proceso_nombre}</h1>
            <p className="text-sm text-slate-500 mt-1">
              Empleado: <strong className="text-slate-700">{aud.evaluado_nombre || '—'}</strong>
              {' · '}Auditor: <strong className="text-slate-700">{aud.auditor_nombre}</strong>
              {' · '}{aud.fecha}
              {' · '}Ejec. <span className="font-mono text-[11px]">{aud.ejecucion_codigo}</span>
            </p>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full transition-all ${aud.aprobada === false ? 'bg-red-500' : (aud.porcentaje >= 70 ? 'bg-emerald-500' : 'bg-amber-500')}`}
              style={{ width: `${aud.porcentaje}%` }}
            />
          </div>
          <span className="text-sm font-semibold text-slate-900 min-w-[60px] text-right" data-testid="audit-pct">{aud.porcentaje}%</span>
          <span className="text-xs text-slate-500 whitespace-nowrap">{aud.puntaje_obtenido}/{aud.puntaje_total} pts · {aud.items_evaluados}/{aud.items_total}</span>
        </div>

        {/* Punto adicional: estado de la supervisión correspondiente */}
        <div
          className={`mt-4 rounded-xl p-3 border flex items-center gap-3 ${aud.supervision_realizada ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}
          data-testid="audit-supervision-check"
        >
          {aud.supervision_realizada
            ? <CheckCircle2 className="w-5 h-5 text-emerald-600 flex-shrink-0"/>
            : <AlertTriangle className="w-5 h-5 text-red-600 flex-shrink-0"/>}
          <div className="flex-1 text-sm min-w-0">
            <p className={`font-semibold ${aud.supervision_realizada ? 'text-emerald-900' : 'text-red-900'}`}>
              Punto adicional · ¿Se realizó la supervisión correspondiente?
            </p>
            <p className={`text-xs ${aud.supervision_realizada ? 'text-emerald-800' : 'text-red-800'}`}>
              {aud.supervision_realizada
                ? <>Sí · <span className="font-mono">{aud.supervision_codigo}</span> {aud.supervision_aprobada === true ? '(aprobada)' : aud.supervision_aprobada === false ? '(reprobada)' : ''}</>
                : aud.supervision_codigo
                  ? <>La supervisión <span className="font-mono">{aud.supervision_codigo}</span> existe pero aún no se ha completado.</>
                  : 'No existe supervisión para esta ejecución. La auditoría no podrá aprobarse hasta completarla.'}
            </p>
          </div>
          {aud.supervision_id && (
            <button
              type="button"
              onClick={() => navigate(`/supervision/${aud.supervision_id}`)}
              className="text-[11px] font-semibold px-2 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 inline-flex items-center gap-1 flex-shrink-0"
              data-testid="audit-open-supervision-btn"
            >
        ...
        {aud.estado === 'completada' && aud.aprobada !== null && aud.aprobada !== undefined && (
          <div
            className={`mt-4 rounded-xl p-3 border flex items-center gap-3 ${aud.aprobada ? 'bg-green-50 border-green-200 text-green-900' : 'bg-red-50 border-red-200 text-red-900'}`}
          ...
          >
            ...
            <div className="flex-1 text-sm">
              <p className="font-bold uppercase tracking-wide">{aud.aprobada ? 'Aprobada' : 'Reprobada'}</p>
              <p className="text-xs opacity-80">
                {aud.aprobada
                  ? `${aud.porcentaje}% (≥70%), sin críticos omitidos y con supervisión completada.`
                  : !aud.supervision_realizada
                    ? `No se realizó la supervisión correspondiente. Reprobada por punto adicional.`
                    : aud.porcentaje < 70
                      ? `${aud.porcentaje}% por debajo del 70% requerido${aud.criticos_omitidos > 0 ? ` · ${aud.criticos_omitidos} crítico(s) omitido(s)` : ''}.`
                      : `${aud.criticos_omitidos} punto(s) crítico(s) omitido(s).`}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-3 flex-wrap">
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Pasos a auditar</h2>
            {items.length > 0 && (
              <button
                onClick={() => setExpandedAll(v => !v)}
                data-testid="audit-toggle-all-details"
            ...
            {!readOnly && items.some(i => i.cumplido !== true) && (
              <button
                onClick={markAllYes}
                disabled={saving}
                data-testid="audit-mark-all-yes"
              ...
              <Fragment key={it.id}>
                <li className={`group transition-colors ${failed ? 'bg-red-50/40' : passed ? 'bg-emerald-50/30' : 'hover:bg-slate-50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-1">
                        ...
                        {it.realizado_comentarios && (
                          <span className="text-[10px] text-slate-500 italic line-clamp-1 max-w-[200px]">&ldquo;{it.realizado_comentarios}&rdquo;</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex-shrink-0 flex items-center gap-1 w-20 justify-end">
                      {canViewEvidence ? (
                        <button
                          ...
                          data-testid={`aud-evidence-${it.id}`}
                  ...
                  {expandedAll && (it.realizado_comentarios || (stepExec?.evidencias?.length > 0) || stepExec?.evidencia) && (
                    <div className="px-6 pb-4" data-testid={`aud-expanded-${it.id}`}>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {(stepExec.evidencias?.length > 0
                                ...
                                <a key={ix} href={ev.data} target="_blank" rel="noreferrer" className="group block rounded-lg overflow-hidden border border-slate-200 bg-white hover:border-blue-300 transition" title={ev.nombre || `Abrir evidencia ${ix + 1}`}>
                                  <img src={ev.data} alt={ev.nombre || `evidencia-${ix + 1}`} className="w-full h-auto block"/>
                                  {ev.nombre && <p className="text-xs text-slate-600 truncate px-3 py-2 border-t border-slate-100 bg-slate-50">{ev.nombre}</p>}
                                </a>
                              ))}
                            </div>
                          </div>
                  ...
                  {failed && (
                    <div className="px-6 pb-4">
                      <div className="bg-white border border-red-200 rounded-xl p-3" data-testid={`aud-plan-${it.id}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <Field label="Desviación">
                            <textarea
                              data-testid={`aud-plan-desviacion-${it.id}`}
                              value={it.desviacion || ''}
                          ...
                          <Field label="Acción correctiva">
                            <textarea
                              data-testid={`aud-plan-accion-${it.id}`}
                          ...
                          <Field label="Responsable">
                            {readOnly ? (
                              ...
                              <select
                                data-testid={`aud-plan-responsable-${it.id}`}
                          ...
                          <Field label="Fecha compromiso">
                            <input
                              type="date"
                              data-testid={`aud-plan-fecha-${it.id}`}
